...
// ---------------------------------------------------------------------------
// Fonts (module state like pageCount — reset per generateExecutivePdf call)
// ---------------------------------------------------------------------------
let FONT_REGULAR = 'Helvetica';
let FONT_BOLD = 'Helvetica-Bold';

// The 3-theme loop re-runs applyPdfFonts per PDF; warn once per message, not 3x
const warnedFontMessages = new Set<string>();
function warnFontOnce(message: string): void {
  if (warnedFontMessages.has(message)) return;
  warnedFontMessages.add(message);
  console.warn(message);
}

function fontErrorHint(fontPath: string, family?: string): string {
  return /\.(ttc|dfont)$/i.test(fontPath) && !family
    ? ' (a .ttc/.dfont collection needs the `family` option naming a face inside it)'
    : '';
}

function applyPdfFonts(doc: PDFKit.PDFDocument, pdfFont?: PdfFontConfig): void {
  FONT_REGULAR = 'Helvetica';
  FONT_BOLD = 'Helvetica-Bold';
  if (!pdfFont) return;
  if (!pdfFont.regular) {
    warnFontOnce('⚠️  pdfFont.regular is empty — falling back to Helvetica');
    return;
  }

  try {
    doc.registerFont('Body', pdfFont.regular, pdfFont.family);
    doc.font('Body'); // force-load now so a bad file falls back instead of failing mid-render
    FONT_REGULAR = 'Body';
  } catch (err) {
    warnFontOnce(`⚠️  PDF font could not be loaded (${pdfFont.regular}): ${err instanceof Error ? err.message : err}${fontErrorHint(pdfFont.regular, pdfFont.family)} — falling back to Helvetica`);
    return;
  }

  FONT_BOLD = 'Body';
  if (pdfFont.bold || pdfFont.boldFamily) {
    const boldPath = pdfFont.bold ?? pdfFont.regular;
    try {
      doc.registerFont('Body-Bold', boldPath, pdfFont.boldFamily);
      doc.font('Body-Bold');
      FONT_BOLD = 'Body-Bold';
    } catch (err) {
      warnFontOnce(`⚠️  PDF bold font could not be loaded (${boldPath}): ${err instanceof Error ? err.message : err}${fontErrorHint(boldPath, pdfFont.boldFamily)} — using the regular font`);
    }
  }
}
...
function drawFooter(doc: PDFKit.PDFDocument, version: string, page: number, scheme: PdfColorScheme, brandingFooter?: string): void {
  ...
  // save()/restore() doesn't stack the font, so pin it — the footer otherwise inherits whatever the last section used
  doc.font(FONT_REGULAR).fontSize(7).fillColor(scheme.lightText);
...
function drawTitleBlock(doc: PDFKit.PDFDocument, data: ExecutivePdfData, version: string, scheme: PdfColorScheme): number {
  ...
  doc.save();
  doc.font(FONT_BOLD).fontSize(22).fillColor(scheme.headingText);
  ...
  doc.save();
  doc.font(FONT_REGULAR).fontSize(11).fillColor(scheme.mutedText);
  ...
  doc.save();
  doc.font(FONT_REGULAR).fontSize(8).fillColor(scheme.lightText);
...
function drawVerdictBanner(doc: PDFKit.PDFDocument, data: ExecutivePdfData, scheme: PdfColorScheme): number {
  ...
  doc.font(FONT_BOLD).fontSize(15).fillColor(textColor);
  doc.text(verdictText, MARGIN + 16, y + 10, { width: CONTENT_W - 32 });

  doc.font(FONT_REGULAR).fontSize(8).fillColor(scheme.mutedText);
...
function drawKpiCards(doc: PDFKit.PDFDocument, data: ExecutivePdfData, scheme: PdfColorScheme): number {
  ...
  for (let i = 0; i < metrics.length; i++) {
    ...
    // Value
    doc.font(FONT_BOLD).fontSize(20).fillColor(m.accent);
    doc.text(m.value, mx + 4, y + 14, { width: cardW - 8, align: 'center' });

    // Label
    doc.font(FONT_REGULAR).fontSize(7.5).fillColor(scheme.mutedText);
...
function drawDistributionSection(doc: PDFKit.PDFDocument, data: ExecutivePdfData, startY: number, scheme: PdfColorScheme): number {
  ...
  // Section title
  doc.save();
  doc.font(FONT_BOLD).fontSize(11).fillColor(scheme.headingText);
  ...
  doc.save();
  doc.font(FONT_BOLD).fontSize(18).fillColor(stats.passRate >= 80 ? scheme.accentGreenDark : scheme.accentRedDark);
  doc.text(`${stats.passRate}%`, donutCx - 28, donutCy - 12, { width: 56, align: 'center' });
  doc.font(FONT_REGULAR).fontSize(7).fillColor(scheme.mutedText);
  ...
  for (let i = 0; i < legendItems.length; i++) {
    ...
    doc.font(FONT_BOLD).fontSize(9).fillColor(scheme.bodyText);
  ...
  if (suiteMap.size > 1) {
    const tableY = ly + 56;
    doc.save();
    doc.font(FONT_BOLD).fontSize(8).fillColor(scheme.bodyText);
    ...
    for (const [file, s] of entries) {
      ...
      doc.font(FONT_REGULAR).fontSize(7).fillColor(scheme.bodyText);
...
function drawQualityGates(doc: PDFKit.PDFDocument, result: QualityGateResult, startY: number, version: string, scheme: PdfColorScheme, footerText?: string): number {
  ...
  doc.font(FONT_BOLD).fontSize(7.5).fillColor(scheme.bodyText);
  ...
  for (let i = 0; i < result.rules.length; i++) {
    ...
    doc.save();
    doc.font(FONT_REGULAR).fontSize(8).fillColor(scheme.bodyText);
  ...
  doc.font(FONT_BOLD).fontSize(9).fillColor(passed ? scheme.accentGreenDark : scheme.accentRedDark);
  doc.text(
...
function drawTrendCharts(doc: PDFKit.PDFDocument, summaries: RunSummary[], startY: number, version: string, scheme: PdfColorScheme, footerText?: string): number {
  ...
  for (let i = 0; i < charts.length; i++) {
    ...
    doc.font(FONT_BOLD).fontSize(14).fillColor(chart.color);
...
function drawFailuresSection(doc: PDFKit.PDFDocument, data: ExecutivePdfData, startY: number, version: string, scheme: PdfColorScheme, footerText?: string): number {
  ...
  for (let i = 0; i < failures.length; i++) {
    ...
    doc.save();
    doc.font(FONT_BOLD).fontSize(7).fillColor(scheme.bodyText);
    ...
    doc.font(FONT_REGULAR).fontSize(6.5).fillColor(scheme.accentRedDark);
...
function drawFailureClusters(doc: PDFKit.PDFDocument, clusters: FailureCluster[], startY: number, version: string, scheme: PdfColorScheme, footerText?: string): number {
  ...
  for (const cluster of clusters.slice(0, 6)) {
    ...
    if (cluster.aiSuggestion) {
      ...
      doc.font(FONT_BOLD).fontSize(6.5).fillColor(scheme.accentBlue);
...
function drawQuarantineSection(doc: PDFKit.PDFDocument, entries: QuarantineEntry[], threshold: number | undefined, startY: number, version: string, scheme: PdfColorScheme, footerText?: string): number {
  ...
  doc.font(FONT_BOLD).fontSize(8).fillColor(scheme.accentAmber);
  ...
  for (let i = 0; i < Math.min(entries.length, 10); i++) {
    ...
    doc.font(FONT_BOLD).fontSize(7).fillColor(scorePct >= 80 ? scheme.accentRedDark : scheme.accentAmber);
...
export function generateExecutivePdf(data: ExecutivePdfData, outputDir: string, basename?: string, theme?: PdfThemeName): string {
  ...
  applyPdfFonts(doc, data.pdfFont);
